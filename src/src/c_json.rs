...
use crate::{
    ...
    __builtin___strcpy_chk, __builtin_object_size, fabs, floor, free, malloc,
    pow, strchr, strlen, strncmp, tolower,
};
use crate::src::c_json_h::{CJSON, CJSONHooks};

static mut c_json_malloc: Option<unsafe extern "C" fn(u64) -> *mut ()> =
    Some(malloc);

static mut c_json_free: Option<unsafe extern "C" fn(*mut ()) -> ()> =
    Some(free);
...
pub(crate) extern "C" fn c_json_init_hooks(hooks: *mut CJSONHooks) -> () {
    unsafe {
        ...
        c_json_malloc =
            if unsafe { (*hooks).malloc_fn.is_some() } {
                unsafe { (*hooks).malloc_fn }
            } else { Some(malloc) };
        c_json_free =
            if unsafe { (*hooks).free_fn.is_some() } {
...
extern "C" fn c_json_new_item() -> *mut CJSON {
    unsafe {
        let node: *mut CJSON =
            unsafe { c_json_malloc.unwrap()(core::mem::size_of::<CJSON>()) }
                as *mut CJSON;
        if !(node).is_null() {
            unsafe {
                __builtin___memset_chk(node as *mut (), 0,
...
extern "C" fn parse_hex4(mut str: *const i8) -> u32 {
    ...
    if unsafe { *str } as i32 >= '0' as i32 &&
            unsafe { *str } as i32 <= '9' as i32 {
        h = h.wrapping_add((unsafe { *str } as i32 - '0' as i32) as u32);
    } else if unsafe { *str } as i32 >= 'A' as i32 &&
            unsafe { *str } as i32 <= 'F' as i32 {
        h = h.wrapping_add((10 + unsafe { *str } as i32 - 'A' as i32) as u32);
    } else if unsafe { *str } as i32 >= 'a' as i32 &&
            unsafe { *str } as i32 <= 'f' as i32 {
        h = h.wrapping_add((10 + unsafe { *str } as i32 - 'a' as i32) as u32);
    } else { return 0 as u32; }
    h = h << 4;
    {
        let __p = &mut str;
        let __t = *__p;
...
extern "C" fn parse_string(item: &mut CJSON, str: *const i8) -> *const i8 {
    unsafe {
        ...
        /// Skip escaped quotes.
        (out =
            unsafe { c_json_malloc.unwrap()((len + 1) as u64) } as *mut i8);
        if (out).is_null() as i32 != 0 { return core::ptr::null(); }
                ...
                '__s2:
                    ...
                    match unsafe { *ptr } {
                        ...
                        117 => {
                            ...
                            if uc >= 56320 as u32 && uc <= 57343 as u32 ||
                                    uc == 0 as u32 {
                                break '__s2;
                            }
                            if uc >= 55296 as u32 && uc <= 56319 as u32 {
                                ...
                                if uc2 < 56320 as u32 || uc2 > 57343 as u32 { break '__s2; }

                                /// invalid second-half of surrogate.
                                (uc =
                                    (65536 as
                                            u32).wrapping_add((uc & 1023 as u32) << 10 |
                                            uc2 & 1023 as u32));
                            }
                            len = 4;
                            if uc < 128 as u32 {
...
#[allow(unused_doc_comments)]
extern "C" fn parse_number(item: &mut CJSON, mut num: *const i8)
    -> *const i8 {
    ...
    if unsafe { *num } as i32 >= '1' as i32 &&
            unsafe { *num } as i32 <= '9' as i32 {
        '__b4: loop {
            '__c4: loop {
                n =
                    n * 10.0 +
            ...
            '__c5: loop {
                {
                    ...
                    { let __p = &mut scale; let __t = *__p; *__p -= 1; __t }
                };
                break '__c5;
            }
            if !(unsafe { *num } as i32 >= '0' as i32 &&
    ...
    if unsafe { *num } as i32 == 'e' as i32 ||
            unsafe { *num } as i32 == 'E' as i32 {
        {
            let __p = &mut num;
            let __t = *__p;
            *__p = unsafe { (*__p).offset(1) };
        ...
        while unsafe { *num } as i32 >= '0' as i32 &&
                unsafe { *num } as i32 <= '9' as i32 {
            subscale =
                subscale * 10 +
                    (unsafe {
    ...
    n =
        sign * n *
            unsafe { pow(10.0, scale + (subscale * signsubscale) as f64) };
...
extern "C" fn skip(mut in__1: *const i8) -> *const i8 {
    while !(in__1).is_null() && unsafe { *in__1 } != 0 &&
            unsafe { *in__1 } as u8 as i32 <= 32 {
        {
            let __p = &mut in__1;
            let __t = *__p;
            *__p = unsafe { (*__p).offset(1) };
...
#[allow(unused_doc_comments)]
extern "C" fn parse_array(item: &mut CJSON, mut value: *const i8)
    -> *const i8 {
    unsafe {
        ...
        /// empty array.
        ((*item).child = { child = c_json_new_item(); child });
        if ((*item).child).is_null() as i32 != 0 { return core::ptr::null(); }

        /// memory fail
        (value = skip(parse_value(child, skip(value))));
        if (value).is_null() as i32 != 0 { return core::ptr::null(); }
        while unsafe { *value } as i32 == ',' as i32 {
            let mut new_item: *mut CJSON = core::ptr::null_mut();
            if ({ new_item = c_json_new_item(); new_item }).is_null() as i32
                    != 0 {
                return core::ptr::null();
            }

            /// memory fail
            unsafe { (*child).next = new_item };
            unsafe { (*new_item).prev = child };
            child = new_item;
            value =
                skip(parse_value(child,
...
#[allow(unused_doc_comments)]
extern "C" fn parse_object(item: &mut CJSON, mut value: *const i8)
    -> *const i8 {
    unsafe {
        let mut child: *mut CJSON = core::ptr::null_mut();
        if unsafe { *value } as i32 != '{' as i32 {
        ...
        while unsafe { *value } as i32 == ',' as i32 {
            ...
            child = new_item;
            value =
                skip(parse_string(unsafe { &mut *child },
                        skip(unsafe { value.offset(1 as isize) })));
            if (value).is_null() as i32 != 0 { return core::ptr::null(); }
            unsafe { (*child).string = unsafe { (*child).valuestring } };
            unsafe { (*child).valuestring = core::ptr::null_mut() };
...
extern "C" fn parse_value(item: *mut CJSON, value: *const i8) -> *const i8 {
    unsafe {
        if (value).is_null() as i32 != 0 { return core::ptr::null(); }
        if (unsafe {
                            strncmp(value, c"null".as_ptr() as *mut i8 as *const i8,
                                4 as u64)
                        } == 0) as i32 != 0 {
            unsafe { (*item).type_ = 2 };
        ...
        if (unsafe {
                            strncmp(value, c"false".as_ptr() as *mut i8 as *const i8,
                                5 as u64)
                        } == 0) as i32 != 0 {
            unsafe { (*item).type_ = 0 };
        ...
        if (unsafe {
                            strncmp(value, c"true".as_ptr() as *mut i8 as *const i8,
                                4 as u64)
                        } == 0) as i32 != 0 {
            unsafe { (*item).type_ = 1 };
        ...
        if unsafe { *value } as i32 == '-' as i32 ||
                unsafe { *value } as i32 >= '0' as i32 &&
                    unsafe { *value } as i32 <= '9' as i32 {
...
pub(crate) extern "C" fn c_json_delete(mut c: *mut CJSON) -> () {
    unsafe {
        ...
        while !(c).is_null() {
            next = unsafe { (*c).next };
            if (unsafe { (*c).type_ } & 256 == 0) as i32 != 0 &&
            ...
            if (unsafe { (*c).type_ } & 256 == 0) as i32 != 0 &&
                ...
                unsafe {
                    c_json_free.unwrap()(unsafe { (*c).valuestring } as *mut ())
                };
            }
            if (unsafe { (*c).type_ } & 512 == 0) as i32 != 0 &&
                    !(unsafe { (*c).string }).is_null() {
                unsafe {
                    c_json_free.unwrap()(unsafe { (*c).string } as *mut ())
                };
            }
...
#[allow(unused_doc_comments)]
pub(crate) extern "C" fn c_json_parse_with_opts(value: *const i8,
    return_parse_end: *mut *const i8, require_null_terminated: i32)
    -> *mut CJSON {
    unsafe {
        let mut end: *const i8 = core::ptr::null();
        let c: *mut CJSON = c_json_new_item();
        ep = core::ptr::null();
        if (c).is_null() as i32 != 0 { return core::ptr::null_mut(); }
...
extern "C" fn ensure(p: *mut Printbuffer, mut needed: i32) -> *mut i8 {
    unsafe {
        ...
        if (p).is_null() as i32 != 0 ||
                (unsafe { (*p).buffer }).is_null() as i32 != 0 {
            return core::ptr::null_mut();
        }
        ...
        if needed <= unsafe { (*p).length } {
            return unsafe {
                    unsafe {
                        (*p).buffer.offset(unsafe { (*p).offset } as isize)
                    }
                };
        }
        newsize = pow2gt(needed);
        newbuffer =
            unsafe { c_json_malloc.unwrap()(newsize as u64) } as *mut i8;
        if (newbuffer).is_null() as i32 != 0 {
            unsafe {
                c_json_free.unwrap()(unsafe { (*p).buffer } as *mut ())
            };
            {
                unsafe { (*p).length = 0 };
        ...
        if !(newbuffer).is_null() {
            unsafe {
                __builtin___memcpy_chk(newbuffer as *mut (),
...
extern "C" fn print_number(item: &CJSON, p: *mut Printbuffer) -> *mut i8 {
    unsafe {
        ...
        if d == 0 as f64 {
            if !(p).is_null() {
                str = ensure(p, 2);
            } else { str = unsafe { c_json_malloc.unwrap()(2) } as *mut i8; }
            if !(str).is_null() {
                unsafe {
                    __builtin___strcpy_chk(str,
                        c"0".as_ptr() as *mut i8 as *const i8,
                        unsafe {
                            __builtin_object_size(str as *const (),
                                if 2 > 1 { 1 } else { 0 })
                        })
                };
            }
        } else if unsafe { fabs((*item).valueint as f64 - d) } <=
                        2.220446049250313e-16 && d <= 2147483647 as f64 &&
                d >= (-2147483647 - 1) as f64 {
            if !(p).is_null() {
                str = ensure(p, 21);
            } else { str = unsafe { c_json_malloc.unwrap()(21) } as *mut i8; }
            if !(str).is_null() {
                unsafe {
                    __builtin___sprintf_chk(str, 0,
                        unsafe {
                            __builtin_object_size(str as *const (),
                                if 2 > 1 { 1 } else { 0 })
                        }, c"%d".as_ptr() as *mut i8 as *const i8, (*item).valueint)
                };
            }
        } else {
            if !(p).is_null() {
                str = ensure(p, 64);
            } else { str = unsafe { c_json_malloc.unwrap()(64) } as *mut i8; }
            if !(str).is_null() {
                if unsafe { fabs(unsafe { floor(d) } - d) } <=
                            2.220446049250313e-16 && unsafe { fabs(d) } < 1e60 {
                    unsafe {
                        __builtin___sprintf_chk(str, 0,
                            unsafe {
                                __builtin_object_size(str as *const (),
                                    if 2 > 1 { 1 } else { 0 })
                            }, c"%.0f".as_ptr() as *mut i8 as *const i8, d)
                    };
                } else if unsafe { fabs(d) } < 1e-6 ||
                        unsafe { fabs(d) } > 1000000000.0 {
                    unsafe {
                        __builtin___sprintf_chk(str, 0,
                            unsafe {
                                __builtin_object_size(str as *const (),
                                    if 2 > 1 { 1 } else { 0 })
                            }, c"%e".as_ptr() as *mut i8 as *const i8, d)
                    };
                } else {
                    unsafe {
                        __builtin___sprintf_chk(str, 0,
                            unsafe {
                                __builtin_object_size(str as *const (),
                                    if 2 > 1 { 1 } else { 0 })
                            }, c"%f".as_ptr() as *mut i8 as *const i8, d)
                    };
                }
            }
        }
...
extern "C" fn print_string_ptr(str: *const i8, p: *mut Printbuffer)
    -> *mut i8 {
    unsafe {
        ...
        let mut token: u8 = 0 as u8;
        {
            ptr = str;
            '__b11: loop {
                if !(unsafe { *ptr } != 0) { break '__b11; }
                '__c11: loop {
                    flag |=
                        if unsafe { *ptr } as i32 > 0 &&
                                        (unsafe { *ptr } as i32) < 32 ||
        ...
        if (flag == 0) as i32 != 0 {
            ...
            if !(p).is_null() {
                ...
                out =
                    unsafe { c_json_malloc.unwrap()((len + 3) as u64) } as
                        *mut i8;
            }
            if (out).is_null() as i32 != 0 { return core::ptr::null_mut(); }
            ptr2 = out;
            unsafe {
                *{
                            let __p = &mut ptr2;
            ...
            unsafe {
                __builtin___strcpy_chk(ptr2, str,
                    unsafe {
                        __builtin_object_size(ptr2 as *const (),
                            if 2 > 1 { 1 } else { 0 })
                    })
            };
        ...
        if (str).is_null() as i32 != 0 {
            if !(p).is_null() {
                out = ensure(p, 3);
            } else { out = unsafe { c_json_malloc.unwrap()(3) } as *mut i8; }
            if (out).is_null() as i32 != 0 { return core::ptr::null_mut(); }
            unsafe {
                __builtin___strcpy_chk(out,
                    c"\"\"".as_ptr() as *mut i8 as *const i8,
                    unsafe {
                        __builtin_object_size(out as *const (),
                            if 2 > 1 { 1 } else { 0 })
                    })
            };
            return out;
        ...
        ptr = str;
        while { token = unsafe { *ptr } as u8; token } != 0 &&
                { let __p = &mut len; *__p += 1; *__p } != 0 {
            if !(unsafe {
                                strchr(c"\"\\\u{8}\u{c}\n\r\t".as_ptr() as *mut i8 as
                                        *const i8, token as i32)
                            }).is_null() {
                { let __p = &mut len; let __t = *__p; *__p += 1; __t };
            } else if (token as i32) < 32 { len += 5; }
            {
                let __p = &mut ptr;
                let __t = *__p;
                *__p = unsafe { (*__p).offset(1) };
            ...
            if unsafe { *ptr } as u8 as i32 > 31 &&
                ...
                '__s14:
                    ...
                    match {
                        ...
                        _ => {
                            unsafe {
                                __builtin___sprintf_chk(ptr2, 0,
                                    unsafe {
                                        __builtin_object_size(ptr2 as *const (),
                                            if 2 > 1 { 1 } else { 0 })
                                    }, c"u%04x".as_ptr() as *mut i8 as *const i8, token as i32)
                            };
                            {
                                let __n = 5;
                                let __p = &mut ptr2;
...
extern "C" fn update(p: *const Printbuffer) -> i32 {
    ...
    str =
        unsafe {
            unsafe { (*p).buffer.offset(unsafe { (*p).offset } as isize) }
        };
    return (unsafe { (*p).offset } as
                    u64).wrapping_add(unsafe { strlen(str as *const i8) }) as
            i32;
}

/// Render an array to text
#[allow(unused_doc_comments)]
extern "C" fn print_array(item: &CJSON, depth: i32, fmt: i32,
    p: *mut Printbuffer) -> *mut i8 {
    unsafe {
        ...
        while !(child).is_null() {
            {
                { let __p = &mut numentries; let __t = *__p; *__p += 1; __t };
        ...
        if !(p).is_null() {
            ...
            (i = unsafe { (*p).offset });
            ptr = ensure(p, 1);
            if (ptr).is_null() as i32 != 0 { return core::ptr::null_mut(); }
            ...
            (entries =
                unsafe {
                        c_json_malloc.unwrap()((numentries as
                                    u64).wrapping_mul(core::mem::size_of::<*mut i8>() as u64))
                    } as *mut *mut i8);
            if (entries).is_null() as i32 != 0 {
                return core::ptr::null_mut();
            }
            unsafe {
                __builtin___memset_chk(entries as *mut (), 0,
                    (numentries as
                            u64).wrapping_mul(core::mem::size_of::<*mut i8>() as u64),
            ...
            while !(child).is_null() && (fail == 0) as i32 != 0 {
                ret =
                    print_value(child, depth + 1, fmt, core::ptr::null_mut());
                unsafe {
                    *entries.offset({
                                        let __p = &mut i;
                                        let __t = *__p;
                ...
                if !(ret).is_null() {
                    len +=
                        unsafe {
                                strlen(ret as
                                                *const i8).wrapping_add(2 as
                                            u64).wrapping_add(if fmt != 0 { 1 } else { 0 } as u64)
                            } as i32;
                } else { fail = 1; }
                child = unsafe { (*child).next };
            }
            if (fail == 0) as i32 != 0 {
                out =
                    unsafe { c_json_malloc.unwrap()(len as u64) } as *mut i8;
            }
            if (out).is_null() as i32 != 0 { fail = 1; }
            if fail != 0 {
                {
                    i = 0;
                    '__b18: loop {
                        if !(i < numentries) { break '__b18; }
                        '__c18: loop {
                            if !(unsafe { *entries.offset(i as isize) }).is_null() {
                                unsafe {
                                    c_json_free.unwrap()(unsafe { *entries.offset(i as isize) }
                                            as *mut ())
                                };
                            }
                            break '__c18;
            ...
            /// Compose the output array.
            unsafe { *out = '[' as i32 as i8 };
            ptr = unsafe { out.offset(1 as isize) };
            unsafe { *ptr = 0 as i8 };
            {
                i = 0;
                '__b19: loop {
                    if !(i < numentries) { break '__b19; }
                    '__c19: loop {
                        tmplen =
                            unsafe {
                        ...
                        unsafe {
                            __builtin___memcpy_chk(ptr as *mut (),
                                unsafe { *entries.offset(i as isize) } as *const (), tmplen,
...
#[allow(unused_doc_comments)]
extern "C" fn print_object(item: &CJSON, mut depth: i32, fmt: i32,
    p: *mut Printbuffer) -> *mut i8 {
    unsafe {
        let mut entries: *mut *mut i8 = core::ptr::null_mut();
        ...
        if (numentries == 0) as i32 != 0 {
            if !(p).is_null() {
                ...
                out =
                    unsafe {
                            c_json_malloc.unwrap()(if fmt != 0 { depth + 4 } else { 3 }
                                    as u64)
                        } as *mut i8;
            }
            if (out).is_null() as i32 != 0 { return core::ptr::null_mut(); }
        ...
        if !(p).is_null() {
            ...
            unsafe { (*p).offset += len };
            child = (*item).child;
            { let __p = &mut depth; let __t = *__p; *__p += 1; __t };
            while !(child).is_null() {
                if fmt != 0 {
                    ptr = ensure(p, depth);
                    if (ptr).is_null() as i32 != 0 {
                ...
                len =
                    if fmt != 0 { 1 } else { 0 } +
                        if !(unsafe { (*child).next }).is_null() { 1 } else { 0 };
                ptr = ensure(p, len + 1);
                if (ptr).is_null() as i32 != 0 {
                    return core::ptr::null_mut();
                }
            ...
            unsafe {
                __builtin___memset_chk(entries as *mut (), 0,
                    (core::mem::size_of::<*mut i8>() as
                            u64).wrapping_mul(numentries as u64),
                    unsafe { __builtin_object_size(entries as *const (), 0) })
            };
            unsafe {
                __builtin___memset_chk(names as *mut (), 0,
                    (core::mem::size_of::<*mut i8>() as
                            u64).wrapping_mul(numentries as u64),
            ...
            while !(child).is_null() {
                unsafe {
                    *names.offset(i as isize) =
                        {
                            str =
                                print_string_ptr(unsafe { (*child).string } as *const i8,
                                    core::ptr::null_mut());
                            str
                        }
                };
                ...
                if !(str).is_null() && !(ret).is_null() {
                    len +=
                        unsafe {
                                strlen(ret as
                                                    *const i8).wrapping_add(unsafe {
                                                strlen(str as *const i8)
                                            }).wrapping_add(2 as
                                            u64).wrapping_add(if fmt != 0 { 2 + depth } else { 0 } as
                                        u64)
                            } as i32;
                } else { fail = 1; }
                child = unsafe { (*child).next };
            ...
            if fail != 0 {
                {
                    i = 0;
                    '__b26: loop {
                        if !(i < numentries) { break '__b26; }
                        '__c26: loop {
                            if !(unsafe { *names.offset(i as isize) }).is_null() {
                                unsafe {
                                    c_json_free.unwrap()(unsafe { *names.offset(i as isize) } as
                                            *mut ())
                                };
                            }
                            if !(unsafe { *entries.offset(i as isize) }).is_null() {
            ...
            unsafe { *ptr = 0 as i8 };
            {
                i = 0;
                '__b27: loop {
                    if !(i < numentries) { break '__b27; }
                    '__c27: loop {
                        if fmt != 0 {
                            {
                                j = 0;
                                '__b28: loop {
                                    if !(j < depth) { break '__b28; }
                                    '__c28: loop {
                                        unsafe {
                                            *{
                                                        let __p = &mut ptr;
                        ...
                        tmplen =
                            unsafe {
                                strlen(unsafe { *names.offset(i as isize) } as *const i8)
                            };
                        unsafe {
                            __builtin___memcpy_chk(ptr as *mut (),
                                unsafe { *names.offset(i as isize) } as *const (), tmplen,
                                unsafe { __builtin_object_size(ptr as *const (), 0) })
                        };
                        {
                            let __n = tmplen;
                        ...
                        unsafe {
                            __builtin___strcpy_chk(ptr,
                                unsafe { *entries.offset(i as isize) } as *const i8,
                                unsafe {
                                    __builtin_object_size(ptr as *const (),
                                        if 2 > 1 { 1 } else { 0 })
                                })
                        };
                        {
...
extern "C" fn c_json_strdup(str: *const i8) -> *mut i8 {
    unsafe {
        ...
        unsafe {
            __builtin___memcpy_chk(copy as *mut (), str as *const (), len,
                unsafe { __builtin_object_size(copy as *const (), 0) })
        };
        return copy;
    }
}
...
extern "C" fn print_value(item: *mut CJSON, depth: i32, fmt: i32,
    p: *mut Printbuffer) -> *mut i8 {
    let mut out: *mut i8 = core::ptr::null_mut();
    if (item).is_null() as i32 != 0 { return core::ptr::null_mut(); }
    if !(p).is_null() {
        '__s30:
            {
            match unsafe { (*item).type_ } & 255 {
                ...
                3 => { out = print_number(unsafe { &*item }, p); }
                4 => { out = print_string(unsafe { &*item }, p); }
                5 => {
                    out =
                        unsafe { print_array(unsafe { &*item }, depth, fmt, p) };
                }
                6 => {
                    out =
                        unsafe { print_object(unsafe { &*item }, depth, fmt, p) };
                }
                _ => {}
            }
        }
        ...
        '__s31:
            ...
            match unsafe { (*item).type_ } & 255 {
                2 => {
                    out =
                        c_json_strdup(c"null".as_ptr() as *mut i8 as *const i8);
                }
                0 => {
                    out =
                        c_json_strdup(c"false".as_ptr() as *mut i8 as *const i8);
                }
                1 => {
                    out =
                        c_json_strdup(c"true".as_ptr() as *mut i8 as *const i8);
                }
                3 => {
                    out =
                        print_number(unsafe { &*item }, core::ptr::null_mut());
                }
                4 => {
                    out =
                        print_string(unsafe { &*item }, core::ptr::null_mut());
                }
                5 => {
                    out =
                        unsafe {
                            print_array(unsafe { &*item }, depth, fmt,
                                core::ptr::null_mut())
                        };
                }
                6 => {
                    out =
                        unsafe {
                            print_object(unsafe { &*item }, depth, fmt,
                                core::ptr::null_mut())
                        };
                }
                _ => {}
...
pub(crate) extern "C" fn c_json_print_unformatted(item: *mut CJSON)
    -> *mut i8 {
    return unsafe { print_value(item, 0, 0, core::ptr::null_mut()) };
}

/// Render a cJSON entity to text using a buffered strategy. prebuffer is a guess at the final size. guessing well reduces reallocation. fmt=0 gives unformatted, =1 gives formatted
pub(crate) extern "C" fn c_json_print_buffered(item: *mut CJSON,
    prebuffer: i32, fmt: i32) -> *mut i8 {
    unsafe {
        let mut p: Printbuffer = unsafe { core::mem::zeroed() };
        p.buffer =
            unsafe { c_json_malloc.unwrap()(prebuffer as u64) } as *mut i8;
        p.length = prebuffer;
        p.offset = 0;
...
pub(crate) extern "C" fn c_json_get_array_item(array: &CJSON, mut item: i32)
    -> *mut CJSON {
    let mut c: *mut CJSON = (*array).child;
    while !(c).is_null() && item > 0 {
        {
            { let __p = &mut item; let __t = *__p; *__p -= 1; __t };
            c = unsafe { (*c).next }
        };
    }
    return c;
...
extern "C" fn c_json_strcasecmp(mut s1: *const i8, mut s2: *const i8) -> i32 {
    if (s1).is_null() as i32 != 0 { return if s1 == s2 { 0 } else { 1 }; }
    if (s2).is_null() as i32 != 0 { return 1; }
    {
        '__b34: loop {
            ...
            '__c34: loop {
                if unsafe { *s1 } as i32 == 0 { return 0; }
                break '__c34;
            }
            {
                {
...
pub(crate) extern "C" fn c_json_get_object_item(object: &CJSON,
    string: *const i8) -> *mut CJSON {
    let mut c: *mut CJSON = (*object).child;
    while !(c).is_null() &&
            c_json_strcasecmp(unsafe { (*c).string } as *const i8, string) !=
                0 {
        c = unsafe { (*c).next };
    }
    return c;
...
pub(crate) extern "C" fn c_json_create_null() -> *mut CJSON {
    ...
    if !(item).is_null() { unsafe { (*item).type_ = 2 }; }
    return item;
}

pub(crate) extern "C" fn c_json_create_true() -> *mut CJSON {
    let item: *mut CJSON = c_json_new_item();
    if !(item).is_null() { unsafe { (*item).type_ = 1 }; }
    return item;
}

pub(crate) extern "C" fn c_json_create_false() -> *mut CJSON {
    let item: *mut CJSON = c_json_new_item();
    if !(item).is_null() { unsafe { (*item).type_ = 0 }; }
    return item;
}
...
pub(crate) extern "C" fn c_json_create_bool(b: i32) -> *mut CJSON {
    let item: *mut CJSON = c_json_new_item();
    if !(item).is_null() {
        unsafe { (*item).type_ = if b != 0 { 1 } else { 0 } };
    }
    return item;
}
...
pub(crate) extern "C" fn c_json_create_string(string: *const i8)
    -> *mut CJSON {
    let item: *mut CJSON = c_json_new_item();
    if !(item).is_null() {
        unsafe { (*item).type_ = 4 };
...
pub(crate) extern "C" fn c_json_create_array() -> *mut CJSON {
    let item: *mut CJSON = c_json_new_item();
    if !(item).is_null() { unsafe { (*item).type_ = 5 }; }
    return item;
}

pub(crate) extern "C" fn c_json_create_object() -> *mut CJSON {
    let item: *mut CJSON = c_json_new_item();
    if !(item).is_null() { unsafe { (*item).type_ = 6 }; }
    return item;
}
...
pub(crate) extern "C" fn c_json_create_int_array(numbers: *const i32,
    count: i32) -> *mut CJSON {
    let mut i: i32 = 0;
    ...
    let a: *mut CJSON = c_json_create_array();
    {
        i = 0;
        '__b36: loop {
            if !(!(a).is_null() && i < count) { break '__b36; }
            '__c36: loop {
                n =
                    c_json_create_number(unsafe { *numbers.offset(i as isize) }
                            as f64);
                if (i == 0) as i32 != 0 {
                    unsafe { (*a).child = n };
...
pub(crate) extern "C" fn c_json_create_float_array(numbers: *const f32,
    count: i32) -> *mut CJSON {
    let mut i: i32 = 0;
    let mut n: *mut CJSON = core::ptr::null_mut();
    let mut p: *mut CJSON = core::ptr::null_mut();
    let a: *mut CJSON = c_json_create_array();
    {
        i = 0;
        '__b37: loop {
            if !(!(a).is_null() && i < count) { break '__b37; }
            '__c37: loop {
                n =
                    c_json_create_number(unsafe { *numbers.offset(i as isize) }
                            as f64);
...
pub(crate) extern "C" fn c_json_create_double_array(numbers: *const f64,
    count: i32) -> *mut CJSON {
    let mut i: i32 = 0;
    let mut n: *mut CJSON = core::ptr::null_mut();
    let mut p: *mut CJSON = core::ptr::null_mut();
    let a: *mut CJSON = c_json_create_array();
    {
        i = 0;
        '__b38: loop {
            if !(!(a).is_null() && i < count) { break '__b38; }
            '__c38: loop {
                n =
                    c_json_create_number(unsafe {
                            *numbers.offset(i as isize)
                        });
                if (i == 0) as i32 != 0 {
                    unsafe { (*a).child = n };
                } else { suffix_object(p, n); }
...
pub(crate) extern "C" fn c_json_create_string_array(strings: *mut *const i8,
    count: i32) -> *mut CJSON {
    let mut i: i32 = 0;
    let mut n: *mut CJSON = core::ptr::null_mut();
    let mut p: *mut CJSON = core::ptr::null_mut();
    let a: *mut CJSON = c_json_create_array();
    {
        i = 0;
        '__b39: loop {
            if !(!(a).is_null() && i < count) { break '__b39; }
            '__c39: loop {
                n =
                    c_json_create_string(unsafe {
...
pub(crate) extern "C" fn c_json_add_item_to_array(array: &mut CJSON,
    item: *mut CJSON) -> () {
    let mut c: *mut CJSON = (*array).child;
    if (item).is_null() as i32 != 0 { return; }
...
pub(crate) extern "C" fn c_json_add_item_to_object(object: *mut CJSON,
    string: *const i8, item: *mut CJSON) -> () {
    unsafe {
        if (item).is_null() as i32 != 0 { return; }
        if !(unsafe { (*item).string }).is_null() {
            unsafe {
                c_json_free.unwrap()(unsafe { (*item).string } as *mut ())
            };
        }
...
pub(crate) extern "C" fn c_json_add_item_to_object_cs(object: *mut CJSON,
    string: *const i8, item: *mut CJSON) -> () {
    unsafe {
        if (item).is_null() as i32 != 0 { return; }
        if (unsafe { (*item).type_ } & 512 == 0) as i32 != 0 &&
...
extern "C" fn create_reference(item: *const CJSON) -> *mut CJSON {
    ...
    if (ref_).is_null() as i32 != 0 { return core::ptr::null_mut(); }
    unsafe {
        __builtin___memcpy_chk(ref_ as *mut (), item as *const (),
            core::mem::size_of::<CJSON>() as u64,
            unsafe { __builtin_object_size(ref_ as *const (), 0) })
    };
    unsafe { (*ref_).string = core::ptr::null_mut() };
    unsafe { (*ref_).type_ |= 256 };
    unsafe {
        (*ref_).next =
            {
...
pub(crate) extern "C" fn c_json_add_item_reference_to_array(array: *mut CJSON,
    item: *mut CJSON) -> () {
    c_json_add_item_to_array(unsafe { &mut *array },
        create_reference(item as *const CJSON));
}

pub(crate) extern "C" fn c_json_add_item_reference_to_object(object:
        *mut CJSON, string: *const i8, item: *mut CJSON) -> () {
    c_json_add_item_to_object(object, string,
        create_reference(item as *const CJSON));
}

/// Remove/Detatch items from Arrays/Objects.
pub(crate) extern "C" fn c_json_detach_item_from_array(array: &mut CJSON,
    mut which: i32) -> *mut CJSON {
    let mut c: *mut CJSON = (*array).child;
    while !(c).is_null() && which > 0 {
        {
            c = unsafe { (*c).next };
            { let __p = &mut which; let __t = *__p; *__p -= 1; __t }
        };
    }
    if (c).is_null() as i32 != 0 { return core::ptr::null_mut(); }
    ...
    if c == (*array).child { (*array).child = unsafe { (*c).next }; }
    unsafe {
        (*c).prev =
            {
...
pub(crate) extern "C" fn c_json_delete_item_from_array(array: *mut CJSON,
    which: i32) -> () {
    c_json_delete(c_json_detach_item_from_array(unsafe { &mut *array },
            which));
}

pub(crate) extern "C" fn c_json_detach_item_from_object(object: *mut CJSON,
    string: *const i8) -> *mut CJSON {
    let mut i: i32 = 0;
...
pub(crate) extern "C" fn c_json_delete_item_from_object(object: *mut CJSON,
    string: *const i8) -> () {
    c_json_delete(c_json_detach_item_from_object(object, string));
}

/// Update array items.
pub(crate) extern "C" fn c_json_insert_item_in_array(array: *mut CJSON,
    mut which: i32, newitem: *mut CJSON) -> () {
    ...
    } else { unsafe { (*unsafe { (*newitem).prev }).next = newitem }; }
}

pub(crate) extern "C" fn c_json_replace_item_in_array(array: &mut CJSON,
    mut which: i32, newitem: *mut CJSON) -> () {
    let mut c: *mut CJSON = (*array).child;
    while !(c).is_null() && which > 0 {
        {
...
pub(crate) extern "C" fn c_json_replace_item_in_object(object: *mut CJSON,
    string: *const i8, newitem: *mut CJSON) -> () {
    let mut i: i32 = 0;
    let mut c: *const CJSON = unsafe { (*object).child } as *const CJSON;
    while !(c).is_null() &&
            c_json_strcasecmp(unsafe { (*c).string } as *const i8, string) !=
...
#[allow(unused_doc_comments)]
pub(crate) extern "C" fn c_json_duplicate(item: *mut CJSON, recurse: i32)
    -> *mut CJSON {
    ...
    if (newitem).is_null() as i32 != 0 { return core::ptr::null_mut(); }

    /// Copy over all vars
    {
        ({
                unsafe { (*newitem).type_ = unsafe { (*item).type_ } & !256 };
    ...
    if !(unsafe { (*item).valuestring }).is_null() {
        unsafe {
            (*newitem).valuestring =
                c_json_strdup(unsafe { (*item).valuestring } as *const i8)
        };
    ...
    if !(unsafe { (*item).string }).is_null() {
        unsafe {
            (*newitem).string =
                c_json_strdup(unsafe { (*item).string } as *const i8)
        };
    ...
    if (recurse == 0) as i32 != 0 { return newitem; }
    ...
    while !(cptr).is_null() {
        ...
        if !(nptr).is_null() {
            ...
            nptr = newchild;
        } else { unsafe { (*newitem).child = newchild }; nptr = newchild; }
        ...
        if unsafe { *json } as i32 == ' ' as i32 {
            ...
            while unsafe { *json } != 0 &&
                    !(unsafe { *json } as i32 == '*' as i32 &&
                                    unsafe { *json.offset(1 as isize) } as i32 == '/' as i32) as
                            i32 != 0 {
                {
                    let __p = &mut json;
                    let __t = *__p;
